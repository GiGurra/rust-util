...
use crate::k8s::ApiResourceType;
use crate::shell::RunCommandError;
...
pub fn api_resource_types() -> Result<ApiResourceTypes, RunCommandError> {
    let result = crate::shell::run_command(
    ...
    result.map(|command_result_output| {
        ...
        let line_maps: Vec<HashMap<String, String>> = crate::string::parse_stdout_table(&lines);
...
fn map_to_resource_type(map: &HashMap<String, String>) -> ApiResourceType {
    ApiResourceType {
        ...
        short_names: crate::string::split_to_vec(&map["SHORTNAMES"], ",", true),
        namespaced: bool::from_str(&map["NAMESPACED"]).expect("non-bool 'NAMESPACED' in map"),
        kind: String::from(&map["KIND"]),
        verbs: crate::string::split_to_vec_r(crate::string::remove_wrap(&map["VERBS"]), &Regex::new(r"\s+").expect("BUG: invalid regex to split VERBS"), true),
